...
| NodeGpuTopology ClusterWriter（§7.1：controller-runtime 对接 CRD + 创建/更新 + 采集失败标记不健康 T2 + allocatedTo 回填） | `pkg/agent/cluster_writer.go` | ✅ 已实现 + 单测（fake client） |
| topo-agent Pod 对账回填（§7.3.3 校正路径：监听 gpu-uuids annotation -> 回填 allocatedTo） | `pkg/agent/pod_reconciler.go` | ✅ 已实现 |
| 对账漂移分类处置（§7.3.3 N2：记账超前核对清理 / 物理占用超前 locked 安全阀，不篡改记账） | `pkg/allocator/reconcile.go` | ✅ 已实现 + 单测 |
| agent 集群模式入口（`--writer=cluster`：采集 + PodReconciler 对账） | `cmd/agent` | ✅ 已实现 |
